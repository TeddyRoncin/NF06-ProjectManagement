...
class TreeWidget(Widget):
    ...
    def generate_widgets(self, start, x, y, length_differences_with_others=()):
        self.task_widgets.append(self.generate_tree_task_widget(start, (x + self.bb.x, y + self.bb.y), lambda: self.position_offset, self.get_bb))
        while len(start.downstream_tasks):
            ...
            if len(start.downstream_tasks) != 1:
                ...
                for downstream_task in start.downstream_tasks:
                    ...
                    y += y_increase
                    self.link_widgets.append(self.generate_tree_link_widget((last_x + self.bb.x, last_y + self.bb.y), (x + self.bb.x, y + self.bb.y), lambda: self.position_offset, self.get_bb))
                    self.generate_widgets(downstream_task, x, y, length_differences_with_others=(*length_differences_with_others, start.downstream_tasks_count - 1 - downstream_task.downstream_tasks_count))
                    y += y_increase + 20
                return
            new_start = start.downstream_tasks[0]
            print("coucou")
            print(new_start)
            print(new_start.upstream_tasks)
            if new_start.upstream_tasks[0] != start:
                widget = [widget for widget in self.task_widgets if widget.task == new_start][0]
                self.link_widgets.append(self.generate_tree_link_widget((last_x + self.bb.x, last_y + self.bb.y), (widget.bb.x + widget.bb.width / 2, widget.bb.y + widget.bb.height / 2), lambda: self.position_offset, self.get_bb))
                return
